...
interface CollaborateurRow {
  ...
  logement_type_occupation: 'mixte' | 'fille' | 'garçon' | null;
  chambre_nom: string | null;
  logement_ville: string | null;
  animal: boolean;
}
...
export default async function CollaborateursPage() {
  let collaborateurs: CollaborateurRow[] = [];
  ...
  try {
    ...
    collaborateurs = result.rows as CollaborateurRow[];
  } catch (error) {
  ...
  const stats = {
    ...
    femmes: collaborateurs.filter((c) => c.genre === 'F').length,
    hommes: collaborateurs.filter((c) => c.genre === 'M').length,
    actifs: collaborateurs.filter((c) => {
